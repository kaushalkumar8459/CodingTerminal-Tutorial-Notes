# Day 020 — Undo/Redo Editor — Detailed Solution

## What to Build

History stacks and coalescing.

## Core Implementation / Algorithm

```js
function history(initial){const past=[],future=[];let current=initial;return {set(v){past.push(current);current=v;future.length=0;},undo(){if(past.length){future.push(current);current=past.pop();}},redo(){if(future.length){past.push(current);current=future.pop();}}};}
```
