# Day 013 — Nested Comments — Detailed Solution

## What to Build

Recursive tree rendering and reply state.

## Core Implementation / Algorithm

```js
function addReply(nodes,parentId,reply){return nodes.map(n=>n.id===parentId?{...n,replies:[...(n.replies??[]),reply]:n.replies?.map(x=>addReply([x],parentId,reply)[0])});}
```
