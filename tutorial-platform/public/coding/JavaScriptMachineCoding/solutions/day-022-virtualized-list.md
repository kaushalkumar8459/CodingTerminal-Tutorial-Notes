# Day 022 — Virtualized List — Detailed Solution
...
```js
function visibleRange(scrollTop,rowHeight,viewport,count){const start=Math.floor(scrollTop/rowHeight);const end=Math.min(count,Math.ceil((scrollTop+viewport)/rowHeight));return {start,end};}
```
