# Day 004 — Search & Autocomplete — Detailed Solution
...
```js
let requestId=0; async function search(q,load){const id=++requestId;const data=await load(q);return id===requestId?data:null;}
```
