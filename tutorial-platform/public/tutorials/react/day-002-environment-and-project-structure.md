...
- **Vite** = build/dev tool (fast dev server, bundling). **React** = UI library. **npm** = package manager/script runner. Three different layers, often confused.
- `package.json`: `dependencies` (needed at runtime) vs `devDependencies` (needed only for development/build). `package-lock.json` pins the resolved dependency tree.
- Render flow: `index.html` has `<div id="root">` → `main.jsx` calls `createRoot(rootEl).render(<App />)` → React mounts `App` inside that root.
- **StrictMode**: development-only tool that double-invokes some logic to help surface bugs (impure renders, missing Effect cleanup); does not affect production behavior.
- **`src` vs `public`**: `src` assets are imported and processed by the build tool; `public` assets are served as-is by URL (e.g. `/favicon.ico`).
- Folder conventions (`components`, `pages`, `hooks`, `services`) are **team conventions, not React rules** — structure should match project complexity.
- `node_modules` and `dist` are generated artifacts → belong in `.gitignore`. `.env` files may hold secrets → keep sensitive ones out of Git too.
- Deleting boilerplate safely means also removing its imports (e.g., deleting `App.css` but forgetting `import "./App.css"` breaks the build).

**Rapid-fire answers**

| Question                                    | One-line Answer                                     |
| ------------------------------------------- | --------------------------------------------------- |
| Is Vite the same as React?                  | No — Vite is build tooling, React is the UI library |
| Where do runtime packages go?               | `dependencies`                                      |
| Where do build/test-only tools go?          | `devDependencies`                                   |
| What connects `index.html` to `App`?        | `main.jsx` via `createRoot().render()`              |
| `src` or `public` for imported images?      | `src`                                               |
| Should `node_modules` be committed?         | No, gitignore it                                    |
| Are `components`/`pages` folders mandatory? | No, they're conventions                             |
